...
Function Discover-AlertRule
{
	...
	$decodedId=[System.Web.HttpUtility]::UrlDecode($Id)
	$objInstance.AddProperty("$MPElement[Name='QND.Azure.Monitor.AlertRule.Class']/Id$", $decodedId)	
    ...
    Log-Event -eventID $SUCCESS_EVENT_ID -eventType $EVENT_TYPE_INFORMATION -level $TRACE_VERBOSE -msg ('Posting discovery data for ''{0}'' type {2} for subscription {1} with id {3}' -f $AlertName, $SubscriptionId, $AlertType, $decodedId)
}
...
try {
	...
	do {
		...
		foreach($rule in $result.Values) {
			if($rule.properties.Enabled) {
				Discover-AlertRule -Id $rule.Id -AlertName $rule.Name -AlertType $rule.Type -AlertDescription $rule.properties.Description -AlertKind $rule.Kind -Location $rule.Location -SubscriptionId $SubscriptionId
			}
		}
	} while ($nextLink)
				   ...
				   if ($actions.Values) {
						foreach($action in $actions.Values) {
							if ($action.properties.Type -ieq 'Alert') {
								...
								Discover-AlertRule -Id $schedule.Values[0].id -AlertName $action.properties.Name -AlertDescription $AlertDescription -AlertType 'Microsoft.OperationalInsights' -AlertKind $alertType -Location 'n.a.' -SubscriptionId $SubscriptionId
							}
						}
				   }
				}
