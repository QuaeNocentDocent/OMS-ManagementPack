...
$SCRIPT_NAME	= "GetAzureMonitorSummary"
$SCRIPT_VERSION = "1.0"
...
try
{
    ...
    foreach($key in $sevHash.Keys) {
        $returnValue.Add($key, $sevHash[$key].new)
        Return-Bag -object $returnValue -key ItemId
    }

    $monitorHash = ConvertTo-HashTable -object $result.Values[0].properties.summaryByMonitorService
    foreach($key in $monitorHash.Keys) {
        $returnValue=@{
            'Type' = 'MonitorService'
            'TypeValue' = $key
            'Count'=$monitorHash[$key]
        }
        Return-Bag -object $returnValue -key ItemId        
    }
